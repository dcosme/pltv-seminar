...
#### statistics {.tabset}
##### norms
The difference between the experimental and control groups is statistically unlikely to be due simply to chance. 

```{r}
lmerTest::lmer(value ~ group + (1 | ResponseId),
                       data = filter(messages, grepl("check norms", name))) %>%
...
```{r}
lmerTest::lmer(value ~ group + (1 | ResponseId),
                       data = filter(messages, grepl("check identity", name))) %>%
  broom.mixed::tidy(., conf.int = TRUE) %>%
  filter(term == "groupexperimental")
```
...
#### statistics {.tabset}
##### self-relevance
The difference between the experimental and control groups is statistically likely to be due simply to chance. As such, we can say that the groups probably don't differ from each other.

```{r}
lmerTest::lmer(value ~ group + (1 | ResponseId),
                       data = filter(messages, grepl("relevance self", name))) %>%
  broom.mixed::tidy(., conf.int = TRUE) %>%
  filter(term == "groupexperimental")
```
...
```{r}
lmerTest::lmer(value ~ group + (1 | ResponseId),
                       data = filter(messages, grepl("relevance others", name))) %>%
  broom.mixed::tidy(., conf.int = TRUE) %>%
  filter(term == "groupexperimental")
```
...
```{r}
lmerTest::lmer(value ~ group + (1 | ResponseId),
                       data = filter(messages, grepl("motivation self", name))) %>%
  broom.mixed::tidy(., conf.int = TRUE) %>%
  filter(term == "groupexperimental")
```
...
```{r}
lmerTest::lmer(value ~ group + (1 | ResponseId),
                       data = filter(messages, grepl("motivation others", name))) %>%
  broom.mixed::tidy(., conf.int = TRUE) %>%
  filter(term == "groupexperimental")
```
...
```{r}
lmerTest::lmer(value ~ group + (1 | ResponseId),
                       data = filter(messages, grepl("broadcast", name))) %>%
  broom.mixed::tidy(., conf.int = TRUE) %>%
  filter(term == "groupexperimental")
```
...
```{r}
lmerTest::lmer(value ~ group + (1 | ResponseId),
                       data = filter(messages, grepl("narrowcast", name))) %>%
  broom.mixed::tidy(., conf.int = TRUE) %>%
  filter(term == "groupexperimental")
```
